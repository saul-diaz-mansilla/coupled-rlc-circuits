...
R = 1700
L = 1
C_val = np.linspace(1e-8, 1e-7, 10)
C3 = 5e-8
V0 = 10.3
beta = R / (2 * L)
f0 = 14.1184
...
w2_val = []
dw2 = []
# Loop over different capacitance values
for i in range(10):
    C = C_val[i]
    w1 = np.sqrt(1 / (L * C))
    w2 = np.sqrt(1 / (L * C) + 2 / (L * C3))
    number = str(i+10).zfill(4)
    ...
    # plt.plot(f, fit_2(f, -5, w1, w2, beta, V0), label='Theoretical')
    # plt.title("Fit")
...
C_plot = np.linspace(1e-8, 1e-7, 100)

plt.figure(figsize=(12,5))

# Plot resonance frequency w1 vs capacitance
plt.subplot(121)
plt.errorbar(C_val, w1_val, yerr=dw1, fmt='o', label='Experimental')
# plt.plot(C3_val, w1_val, 'o', label='Experimental')
plt.plot(C_plot, np.sqrt(1/L/C_plot), label='Teórico')
plt.xlabel(r"$C$ (F)")
...
# Plot resonance frequency w2 vs capacitance
plt.subplot(122)
plt.errorbar(C_val, w2_val, yerr=dw1, fmt='o', label='Experimental')
# plt.plot(C3_val, w2_val, 'o', label='Experimental')
plt.plot(C_plot, np.sqrt(1/L/C_plot + 2/L/C3), label='Teórico')
plt.xlabel(r"$C$ (F)")
