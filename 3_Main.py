...
# Amplitude response for capacitor 1
...
# Amplitude response for capacitor 2
...
# Phase response for capacitor 1
...
# Phase response for capacitor 2
...
# Loop to process data for Capacitor 1
for i in range(46):  # 46
    number = str(i).zfill(4)

    CH1 = pd.read_csv(r"RLCC_C1\ALL" + number + r"\F" + number + r"CH1.CSV", header=None)
    CH2 = pd.read_csv(r"RLCC_C1\ALL" + number + r"\F" + number + r"CH2.CSV", header=None)
    ...
    parameters_1, covariance_1 = curve_fit(seno, time_1, volt_1, p0=[(max(volt_1)-min(volt_1))/2, omega, phase_1 % (2*np.pi), np.mean(volt_1)])
    parameters_2, covariance_2 = curve_fit(seno, time_2, volt_2, p0=[(max(volt_2)-min(volt_2))/2, omega, phase_2 % (2*np.pi), np.mean(volt_2)])
...
fig1.errorbar(w, np.abs(V_C1), xerr=d_w, yerr=dV_C1, fmt='.', label="Data C1")
fig1.plot(w_fit, A1(w_fit, parameters_C1[0], parameters_C1[1], parameters_C1[2], parameters_C1[3], parameters_C1[4]), label="Fit C1")
ph_in_1 = np.unwrap(np.array(ph_in_1))
d_dph_1 = np.array(d_ph_in_1) + np.array(d_ph_out_1)
ph_out_1 = np.unwrap(np.array(ph_out_1))
dph_1 = (ph_out_1 - ph_in_1) % (2*np.pi)
dph_1 = np.unwrap(dph_1) % (np.pi)

parameters_ph1, _ = curve_fit(theta1, w, dph_1, p0=[R,L,C,C3,V0], bounds=([0, 0, 0, 0, 0], [np.inf, np.inf, np.inf, np.inf, np.inf]))

fig2.errorbar(w, dph_1, xerr=d_w, yerr=d_dph_1, fmt='.', label="Data C1")
fig2.plot(w_fit, theta1(w_fit, parameters_ph1[0], parameters_ph1[1], parameters_ph1[2], parameters_ph1[3], parameters_ph1[4]), label="Fit C1")
...
DC_0 = []
DC_C2 = []
# Loop to process data for Capacitor 2
for i in [i for i in range(55) if i != 5]:  # 55
    number = str(i).zfill(4)

    CH1 = pd.read_csv(r"RLCC_C2\ALL" + number + r"\F" + number + r"CH1.CSV", header=None)
    CH2 = pd.read_csv(r"RLCC_C2\ALL" + number + r"\F" + number + r"CH2.CSV", header=None)
    ...
    # Fit sinusoidal curves to voltage data
    parameters_1, covariance_1 = curve_fit(seno, time_1, volt_1, p0=[(max(volt_1)-min(volt_1))/2, omega, phase_1, np.mean(volt_1)])
    parameters_2, covariance_2 = curve_fit(seno, time_2, volt_2, p0=[(max(volt_2)-min(volt_2))/2, omega, phase_2, np.mean(volt_2)])
    err_1 = np.sqrt(np.diag(covariance_1))
    err_2 = np.sqrt(np.diag(covariance_2))

    V_0.append(parameters_1[0])
    V_C2.append(parameters_2[0])
    dV_C2.append(err_2[0])
    w.append(parameters_1[1])
    d_w.append(err_1[1])
    ph_in_2.append(parameters_1[2])
    d_ph_in_2.append(err_1[2])
    ph_out_2.append(parameters_2[2])
    d_ph_out_2.append(err_2[2])
    DC_0.append(parameters_1[3])
    DC_C2.append(parameters_2[3])

parameters_C2, covariance_C2 = curve_fit(A2, w, np.abs(V_C2), p0=[R,L,C,C3,V0])

fig1.errorbar(w, np.abs(V_C2), xerr=d_w, yerr=dV_C2, fmt='.', label="Data C2")
fig1.plot(w_fit, A2(w_fit, parameters_C2[0], parameters_C2[1], parameters_C2[2], parameters_C2[3], parameters_C2[4]), label="Fit C2")

ph_in_2 = np.unwrap(np.array(ph_in_2))
ph_out_2 = np.unwrap(np.array(ph_out_2))
dph_2 = (np.array(ph_out_2) - np.array(ph_in_2)) % (2*np.pi)
dph_2 = np.unwrap(dph_2) % (np.pi)
d_dph_2 = np.array(d_ph_in_2) + np.array(d_ph_out_2)

parameters_ph2, _ = curve_fit(theta2, w, dph_2, p0=[R,L,C,C3,V0], bounds=([0, 0, 0, 0, 0], [np.inf, np.inf, np.inf, np.inf, np.inf]))
fig2.errorbar(w, dph_2, xerr=d_w, yerr=d_dph_2, fmt='.', label="Data C2")
fig2.plot(w_fit, theta2(w_fit, parameters_ph2[0], parameters_ph2[1], parameters_ph2[2], parameters_ph2[3], parameters_ph2[4]), label="Fit C2")

fig1.set_title("Capacitor Amplitudes")
fig1.set_xlabel(r"$\omega$ (rad/s)")
fig1.set_ylabel(r"$V$ (V)")
fig1.legend()
fig1.grid()

fig2.set_title("Phase Difference")
fig2.set_xlabel(r"$\omega$ (rad/s)")
fig2.set_ylabel(r"$\Delta \phi$ (rad)")
fig2.legend()
fig2.grid()
